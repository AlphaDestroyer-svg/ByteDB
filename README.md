...
- ✅ Transactions: `BEGIN [SERIALIZABLE]`, `COMMIT`, `ROLLBACK`, MVCC snapshot isolation; `SERIALIZABLE` enforces true serializability via SSI (rw-antidependency tracking + dangerous-structure abort) — prevents write skew and predicate/phantom anomalies that plain SI allows
...
- ✅ Транзакции: `BEGIN [SERIALIZABLE]`, `COMMIT`, `ROLLBACK`, MVCC snapshot isolation; `SERIALIZABLE` даёт настоящую сериализуемость через SSI (трекинг rw-antidependency + abort на «опасной структуре») — исключает write skew и predicate/phantom аномалии, которые допускает обычный SI
